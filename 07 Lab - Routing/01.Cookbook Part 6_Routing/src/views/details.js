...
const detailsTemplate = (recipe, isOwner, onDelete) => html` <section id="details">
    ${recipeCard(recipe, isOwner, onDelete)} ${showComments(recipe)}
</section>`;

const recipeCard = (recipe, isOwner, onDelete) => html` <article>
    <h2>${recipe.name}</h2>
    <div class="band">
...
export function setupDetails() {
    ...
    async function showDetails(context) {
        ...
        return detailsTemplate(recipe, isOwner, () =>
            onDelete(recipe, () => context.page.redirect('/deleted/' + id))
        );
    }
